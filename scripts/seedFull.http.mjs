...
async function ensureWaitlistEntry(entryData, cookie = "") {
  const { name, email, status } = entryData;

  // 1. Check if email already exists
  const q = `query($email:String!){ waitlistEntries(where:{email:{equals:$email}}, take:1){ id email } }`;
  const found = await getOne(q, { email: email }, cookie, "waitlistEntries");

  if (found && found.length) {
    console.log(`Waitlist entry for ${email} already exists. Skipping creation.`);
    return found[0]; // Return the first item in the array
  }

  // 2. Create if not found
  const m = `mutation($d:WaitlistEntryCreateInput!){
    createWaitlistEntry(data:$d){
      id
      name
      email
      status
  ...
  const { data } = await gql(
    m,
    { d: { name, email, status: status || "pending" } }, // set to pending if not provided
    cookie
  );

  console.log(`Created waitlist entry:`, data.createWaitlistEntry);
  return data.createWaitlistEntry;
}

async function seedWaitlist() {
  try {
    console.log("Seeding waitlist with 3 entries...");

    // Create three waitlist entries
    await ensureWaitlistEntry({
      name: "Alice Smith",
    ...
    console.log("Waitlist seeding complete.");
  } catch (error) {
